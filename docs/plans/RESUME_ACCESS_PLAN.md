...
- `public/resume.pdf` exists, is tracked, and is a four-page PDF.
- `professionalContent.resume` provides the label `Download Resume`, URL `/resume.pdf`, and updated date `2026-08-13`.
...
- `public/resume.pdf` is the approved current public CV. This task exposes the existing artefact rather than changing its editorial content.
...
2. The page exposes an accessible `Download Resume` link sourced from `professionalContent.resume` and resolving to `/resume.pdf`.
...
5. The production build includes the PDF at the expected root URL.
   - Evidence: successful build and verification that `dist/resume.pdf` exists.
...
- `dist/resume.pdf` exists after the production build.
